    ...
    <div class="mt-2" v-else>
        ...
        <div class="row mt-3">
            <div v-for="t, key in title" :key="key" :class="`col-sm-${t.length}`" >
                <b v-if="t.type != 'buttonModal' && t.length != 'hidden'" class="text-white">{{ t.title }}</b>
            </div>
        </div>
        <hr>
        <div class="row mt-4 border-bottom resultado-consultas-row row-list" v-for="obj, key in filteredData" :key="key">
            <div :class="`col-sm-${title[keyValue].length}`" v-for="value, keyValue in obj" :key="keyValue">
                ...
                <span v-if="keyValue == 'profile'" class="text-white"> {{ value | formatProfile }}</span>
                <span v-if="title[keyValue].type == 'datetime' && title[keyValue].length != 'hidden'" class="text-white"> {{ value | formatDateTime }}</span>
                <button v-if="title[keyValue].type == 'buttonModal' && title[keyValue].length != 'hidden'" class="btn btn-sm btn-outline-light w-100" data-bs-toggle="modal" :data-bs-target="title[keyValue].modalId" @click="setStore(obj)">
